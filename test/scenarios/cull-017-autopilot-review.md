...
dismiss/undo. `runAutopilot` (`AppModel.swift:9539`) plans in-memory
...
`AppModel.swift:9600`), tagged `origin=ai` (`aiUnconfirmedFields` contains
...
`AppModel.swift:9705` `dismissAutopilotRunSummary`) → grid-cell KEEP/CUT
...
(`AppModel.swift:9753`) being the gesture that *confirms* the ghost (clears
`aiUnconfirmedFields`, writes the sidecar) rather than first-writing
anything, and `dismissAutopilotProposals(assetIDs:)` (`AppModel.swift:9799`)
...
- **This card drives the post-import armed-Autopilot path, not the on-demand
  gesture.** `runAutopilot` (`AppModel.swift:9539`) has two entry points: an
...
6. **Review is reachable after Dismiss — the open question this card used to
   ...
   (`AppModel.swift:4695`) → `selectSource(_:)` (`AppModel.swift:4744`) →
   `applySource`'s `.autopilotSuggestions` case →
   `applyAutopilotSuggestionsScope()` (`AppModel.swift:9727`), which sets
   ...
   `beginAutopilotReview()`, `AppModel.swift:9718`, which additionally
...
- **Dismiss (review toolbar), documented but not driven live by this card**:
  `dismissAutopilotProposals(assetIDs:)` (`AppModel.swift:9799`) records
...
- **Resolved (step 6): Review stays reachable after the banner's Dismiss.**
  ...
  call `beginAutopilotReview()` (`AppModel.swift:9718`, which additionally
  ...
  (`AppModel.swift:4695,4744,9727`) without touching the lens. Dismissing
...
(`AppModel.swift:4695`) → `selectSource(_:)` (`AppModel.swift:4744`) →
`applySource`'s `.autopilotSuggestions` case →
`applyAutopilotSuggestionsScope()` (`AppModel.swift:9727`), a second,
independent path into review state alongside `beginAutopilotReview()`
(`AppModel.swift:9718`, the banner Review button's and
