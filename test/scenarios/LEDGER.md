...
| cull-014-stack-rail | cull-014-stack-rail.md | Needs re-verify | VM e2e | — | prior burst re-run PASS predates Task 7's ellipsis-menu collapse of secondary actions | Task 7 (2026-07-11): secondary actions moved into an ellipsis menu; card updated, not re-driven |
| cull-015-sidebar-sources | cull-015-sidebar-sources.md | Needs re-verify | VM e2e (ax+sql) | — | prior run predates Task 7's diagnostics-fold-in + omit-not-disable + empty-state change | Task 7 (2026-07-11): Diagnostics disclosure removed (rows fold into "Cull From"), zero-count rows omitted not disabled, "Nothing to cull" empty state added; card updated, not re-driven |
