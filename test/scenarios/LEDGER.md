...
| cull-002-loupe-navigation | cull-002-loupe-navigation.md | Fixed | VM e2e | — | arrows L/R/Space PASS; Up/Down+⌥arrows "inert" is DESIGNED no-op on all-singleton catalogs (selectCullingStack jumps multi-frame stacks only; dispatch verified reaching the model) | card steps 6-7/Expected corrected 2026-07-10 to assert the no-op; real stack-to-stack nav still needs a multi-frame fixture |
| cull-003-rating-label-flag-keys | cull-003-rating-label-flag-keys.md | Fixed | VM e2e | Functional | root cause: bare Culling-menu key equivalents fire via performKeyEquivalent independently of the key monitor (monitor nil-return does not stop menu dispatch) → double dispatch; fixed 3f426d3c: monitors are sole owners, menu binds no bare equivalents; VM-verified 1 press = 1 write + 1 advance | needs fresh full card re-run to Verify |
...
| cull-005-scope-cycle | cull-005-scope-cycle.md | Fixed | VM e2e | Functional | same menu double-dispatch root as cull-003; fixed 3f426d3c; VM-verified S single-steps (All→Unrated, 4-press full cycle) | needs fresh full card re-run to Verify |
| cull-006-zoom-and-face-zoom | cull-006-zoom-and-face-zoom.md | Tested-Fail | VM e2e | Testability | tooling unblocked 2026-07-10: vm_scenario_run.sh setup now grants Screen Recording (TCC row + replayd picker-bypass approval for sshd-session); ssh `screencapture -x` verified dialog-free | screenshot-diff method now runnable; card itself needs a fresh run |
| cull-007-exif-overlay-cycle | cull-007-exif-overlay-cycle.md | Fixed | VM e2e | Functional | same menu double-dispatch root; fixed 3f426d3c; VM-verified I lands on exposure-only first | needs fresh full card re-run to Verify |
| cull-008-subview-keys-gcb | cull-008-subview-keys-gcb.md | Fixed | VM e2e | Functional | root cause: View-menu bare g/c/b equivalents re-set selectedView ~150ms after the grid monitor switched — grid→loupe flipped straight back to cullGrid; fixed 3f426d3c (equivalents removed); VM-verified G and Esc both return grid→loupe | needs fresh full card re-run to Verify |
| cull-009-keymap-overlay | cull-009-keymap-overlay.md | Fixed | VM e2e | Functional | root cause: Esc consumed by grid monitor as returnToGrid → app left Cull under the overlay, gating off the ? toggle (overlay genuinely stuck, not an AX ghost); fixed (Esc-derived grid commands dismiss the overlay while visible, KeyMapOverlayDismissTests); VM-verified Esc removes overlay from AX tree | needs fresh full card re-run to Verify |
| cull-010-cullgrid-keys | cull-010-cullgrid-keys.md | Tested-Fail | VM e2e | Testability | nav PASS; tooling unblocked 2026-07-10: ax_drive.sh press --modifiers shift[,command,...] posts CGEvent clicks at the element center; live-verified shift-click range-selected 4 tiles in cullGrid | batch-rating steps now runnable; card needs a fresh run |
| cull-011-hud | cull-011-hud.md | Tested-Pass | VM e2e (ax+sql) | — | counts track catalog exactly |  |
| cull-012-closeups-panel | cull-012-closeups-panel.md | Tested-Fail | VM e2e | Environment | root-caused 2026-07-10, two stacked causes: (1) faces originals were never synced to the VM (original_path baked to host repo path; loupe read "Original missing") — sync/launch now ship photos and rewrite the prefix; (2) face detection is not passive — needs Culling ▸ Evaluate Scope (card updated). Live-verified: 11 face_observations within ~10s | open q: Close-Ups re-detects live vs face_observations table — counts may disagree; intended? |
...
| cull-018-compare-survey | cull-018-compare-survey.md | Tested-Pass | VM e2e | — | core PASS; contenders toggle inert on --smoke is BY DESIGN: .disabled(!isContendersModeAvailable), which needs evaluation_signals ranks — zero on the smoke seed; card step 5 clarified 2026-07-10 | open q: shared monitor key semantics in compare; CONFIRMED UX inconsistency: Return uses stricter stack-guard and can silently no-op while Keep-primary button is enabled |
