...
**What this covers**: the Timeline lens
(`TimelineWorkspaceView` in `Sources/TeststripApp/LibraryGridView.swift:7745-7804`)
...
## Steps
1. `script/ax_drive.sh wait-vended Teststrip`; press ⌘4 for the Timeline
   lens (`LibraryLens.swift:42-53`).
2. Assert the year ribbon is present with at least one year bar, and the
   scrubber shows the corresponding month(s) with a focused month
   highlighted (`isFocused` per `TimelinePresentation.scrubber`).
3. Assert a day/month section is visible in the scroll body
   ...
   no pagination gate).
4. Click a different month row in the scrubber (or a different year bar if
   ...
   scroll.
5. Click a day row/day section header. Assert `selectTimelineDay` fires
...
## Expected
- Step 2: year ribbon renders with `≥1` bar; scrubber's focused month matches
...
- Step 4: **Fails if** clicking a scrubber row changes the model's selection
  ...
  manually.
- Step 5: **Fails if** the "Loaded" count doesn't drop to the selected day's
...
- The Timeline view is one of the surfaces gated behind
  ...
  broken wiring.
- Timeline is a browse lens and therefore retains search, filters, import,
  footer, Cull, Export, and More chrome
  (`LibraryGridView.swift:8254-8315`); this card covers the Timeline-specific
  ribbon/scrubber/scroll behavior, not that shared chrome policy.
