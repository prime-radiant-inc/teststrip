...
**Verified** on the Tart VM `teststrip-e2e` (`script/vm_scenario_run.sh`, `launch smoke`, main@5644c66f). Steps 2-7 PASS on substance; the live View-menu order is: [Show Tab Bar, Show All Tabs] · divider · [Cull ⌘1, Grid ⌘2, Loupe ⌘3, Timeline ⌘4, Map ⌘5, People ⌘6] · divider · [Start Cull Run ⌘R, Loupe (no key), Cull Grid, Compare, A/B Compare] · divider · [Show Inspector ⌘I, Scroll to Info ⌃1, Scroll to Describe ⌃2, Scroll to AI ⌃3] · divider · [Zoom In, Zoom Out, Enter Full Screen]. The lens group equals `LibraryLens.allCases` in order; the four sub-mode items each read no key equivalent. Routing: View▸Timeline / Grid / Cull / Compare all land correctly, and the "Loupe" name collision resolves as designed — the lens-group "Loupe" (menu item 6) selects the Loupe lens with no Cull HUD, while the sub-mode "Loupe" (menu item 12) keeps Cull selected and shows the Cull HUD "6 picks, 5 rejects, 13 left". Bare `c` in Cull single-steps; `c` in Grid is inert (exploratory arm, passes).

**Step 2's "exactly ten titles, two groups separated by a Divider" is stale**: the live View menu has 15 named items in 5 groups — "Start Cull Run" (⌘R, added by SP-D, `main.swift:184-193`) sits between the divider and the sub-mode group, and the card omits the system tab / inspector-scroll / zoom groups. The card's substantive ordering (six lenses, divider, four sub-modes incl. two "Loupes") all hold. Method note: this run read key equivalents via `AXMenuItemCmdChar` / `AXMenuItemCmdModifiers` rather than System Events' `key equivalent` (which errored here), and clicked the second "Loupe" by absolute menu index because a `whose name` filter misrouted to the first.
