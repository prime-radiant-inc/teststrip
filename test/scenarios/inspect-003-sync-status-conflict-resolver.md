...
`Sources/TeststripApp/InspectorView.swift:348-403`).
...
3. Rate `$SRC` 4 stars (`ax_drive.sh press --role AXButton --help "Rate 4"`,
   ...
   kind, `InspectorView.swift:837-846`) — confirm on disk:
   ```bash
...
4. **Force `pending`.** Make the sidecar folder briefly unwritable, then
   ...
   `InspectorView.swift:385-394`) — cross-check:
   ```bash
   ...
   "Retry" button (`InspectorView.swift:891-900`,
...
5. **Priority check**: while both a conflict and pending state could
   ...
   (`InspectorView.swift:348-403`) — Part B directly forces a `conflict` row
...
4. Switch to Info. Assert the sync status line reads "XMP conflict" (red,
   ...
   minimum (`conflictRows`, `InspectorView.swift:444-453`, which only emits
   a row per field when `catalogValue != sidecarValue`,
   `InspectorView.swift:456-458`). Confirm on disk:
   ```bash
...
5. Click "Merge Missing" (`InspectorMetadataConflictActionPresentation`,
   title "Merge Missing", `InspectorView.swift:1438-1446`,
...
8. Switch to Info. Assert the conflict detail text reads "XMP sidecar
   ...
   `InspectorView.swift:435-441`) and that "Merge Missing" and "Use XMP" are
   both disabled (`isEnabled: sidecarMetadataReadable` on both action kinds,
   `InspectorView.swift:1438-1460`) while "Use Catalog" remains enabled
   (`isEnabled: true` unconditionally, `:1447-1452`).
...
statically: `Sources/TeststripApp/InspectorView.swift:308-458`
