...
- Catalog load builds a Sources sidebar section for recorded catalog source roots and for unavailable or questionable originals: offline, missing, moved, and stale. Selecting a source-root row applies the existing folder-prefix filter; selecting an availability row applies the source availability filter. Both paths make NAS/removable source identity reachable without scanning originals on the grid hot path.
