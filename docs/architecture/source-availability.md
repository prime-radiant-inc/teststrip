...
- Source roots can be reconnected from the library toolbar by entering the old cataloged root and the new mounted root. Reconnect updates only assets whose same relative file exists under the new root and matches the catalog fingerprint. It marks those originals online, keeps metadata generation unchanged, refreshes sidebar/source summaries, and moves XMP sync state to the new sidecar path.
...
- Make reconnect suggestions smarter by proposing likely old roots from selected unavailable sources.
