...
- Current slice focused verification: `swift test --filter SmartCollectionBuilderPresentationTests/testRuleRowsPreserveRemovalTargetsFromActiveFilterRows` failed before implementation because builder rule rows did not preserve structured removal targets, then passed after carrying `ActiveLibraryFilterRow` through the builder presentation.
- Last focused unit verification: `swift test --filter 'SmartCollectionBuilderPresentationTests|AppModelTests/testRemovingActiveLibraryFilter|AppModelTests/testApplyingSmartCollectionRule'` passed with 19 tests after adding removable Smart Collection builder rules. Earlier focused verification includes current-scope evaluation queueing, worker activity status, latest-import preview status scoping, queued import progress copy, culling command menu presentation, Search refinement removal, removable Library filter chips, preview queue thumbnail state, inspector sizing, offline reconnect smoke, empty import summaries, source reconnect errors, import cancel affordance, Copilot scope actions, culling stack guidance, latest-import evaluation handoff, People review-card lock-state, generated Search refinements, People scan action, saved-set lifecycle, RAW catalog-only import, batch metadata/XMP, culling, and import-path slices listed below.
- Last broad unit verification: `swift test` passed after adding removable Smart Collection builder rules, with 912 tests, 1 skipped, and 0 failures. Earlier `swift test` passed after bounding current-scope evaluation queueing, with 911 tests, 1 skipped, and 0 failures. Earlier `swift test` passed after correcting user-facing worker activity status, with 909 tests, 1 skipped, and 0 failures.
- Current slice app workflow verification: No foreground app workflow run was performed for the Smart Collection builder rule-removal control because Jesse asked to minimize focus-stealing UI automation while actively using the machine; the change is covered by presentation/app-model tests, SwiftUI compilation, and full `swift test`. `./script/verify_headless_workflows.sh` also passed on 2026-07-06, rebuilding the app/helper and running non-focus-stealing metadata-write, import-preview-drain, source availability, offline reconnect, preview render, local HTTP model smoke, and worker recovery checks.
...
- Current slice: made Smart Collection builder rule rows removable by carrying structured active-filter targets into the builder and routing removal through the existing filter-removal model path.
