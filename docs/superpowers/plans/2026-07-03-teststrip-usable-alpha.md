...
- Current slice focused verification: live selected and batch metadata edits now keep XMP writeback pending for offline/unwritable originals without dispatching doomed worker sync work, while still dispatching online writable batch targets. The selected-offline and batch-offline tests failed against unconditional worker enqueue, then passed after live enqueue paths reused the same retryability guard as load/retry. Earlier focused verification passed for import-completion optional-review cleanup, local import stability, source-availability sidebar tone, inspector source-status copy, culling-stack visual-similarity vectors, multi-label recognition, source-root unavailable counts, culling-stack placeholder cleanup, People dead-UI cleanup, OCR caption suggestions, work-session query scopes, plain selection batch collapse, import completion skipped-file review, import-issue plumbing, latest-import flagged review, People face-review, Search/parser quoted/import parsing, and recognition retry/gates.
