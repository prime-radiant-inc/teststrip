...
- Current slice focused verification: `swift test --filter CullingStackRailPresentationTests/testActionsKeepPrimaryBehaviorSeparateWhenNoStackRankingSignalsExist` failed before the culling-stack placeholder removal because no-signal stacks still exposed disabled `Keep top 2`; `swift test --filter 'CullingStackRailPresentationTests|CullingAssistPresentationTests'` passed after removing that placeholder while preserving signal-backed `Keep top 2` and `Keep recommended` actions. Earlier focused verification passed for People dead-UI cleanup, OCR caption suggestions, work-session query scopes, plain selection batch collapse, import completion skipped-file review, import-issue plumbing, latest-import flagged review, People face-review, Search/parser quoted/import parsing, and recognition retry/gates.
