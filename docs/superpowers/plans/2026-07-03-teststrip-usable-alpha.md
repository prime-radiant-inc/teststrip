...
- Snapshot commit: `859998c Add local preview focus evaluation`
- Product posture: foundation/dev build moving toward usable alpha, not yet a polished photo app.
- Current slice focused verification: `swift test --filter 'EvaluationProviderTests/testLocalImageMetricsProviderEmitsExposureColorAndFocusSignalsFromPreview|EvaluationProviderTests/testLocalImageMetricsFocusScoreReflectsPreviewEdgeDetail|WorkerCommandExecutorTests/testRuntimeConfigurationRegistersLocalImageMetricsProvider'` and `swift test --filter 'EvaluationProviderTests|WorkerCommandExecutorTests'` passed after adding provenance-backed local preview focus evaluation.
...
- Last broad unit verification: `swift test` passed with 796 tests, 1 skipped, and 0 failures after adding local preview focus evaluation.
- Current slice app workflow verification: No additional app workflow run was performed for local preview focus evaluation because it changes worker/core evaluation behavior and is covered by focused provider/worker tests plus full `swift test`; Jesse asked to minimize focus-stealing automation. Recent app smoke verification still launched an isolated catalog with `./script/build_and_run.sh --verify-smoke` and captured a nonblank Library window at `/tmp/teststrip-latest-ui-smoke.png`; route-level Computer Use inspection remained blocked by desktop window detection.
...
- Current slice: added manual People alpha actions for naming selected photos, merging confirmed people, and dismissing selected photos from face-review queues without deleting the underlying evaluation signals.
...
- Evaluation is scaffolding plus early useful providers, not finished face/person/object/aesthetic workflow. The People view now uses real face-signal coverage for review cards, hands those cards off to existing review targets, can request Apple Vision evaluation for visible cached previews, and supports manual naming/merge/dismiss actions for confirmed people state; object labels can be accepted as selected-photo or visible-batch keyword suggestions, but real automatic face clustering, identity recognition, split, face-box-level naming, whole-set batch label review, and broader reprocessing flows are not wired yet.
