...
- Current slice: route the primary `Import Card` action through the user-granted folder panel and prove the sandbox-style worker-disabled card path copies into the destination root, generates previews, avoids worker commands, and persists the destination-root bookmark.
