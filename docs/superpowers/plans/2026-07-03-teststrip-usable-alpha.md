...
- Current slice: added selected-batch and current-scope provisional keyword review so object-label suggestions can be surfaced from selected photos, the full active query, or a saved set beyond the loaded thumbnail page, then accepted through catalog-first/XMP writeback only for matching assets.
