...
- Current slice focused verification: `swift test --filter 'AppModelTests/testRetryPendingMetadataSyncInCurrentScopeQueuesRetryableItemBeyondLoadedWindow|AppModelTests/testCanRetryPendingMetadataSyncInCurrentScopeLooksBeyondLoadedWindow|AppModelTests/testCanRetryPendingMetadataSyncInCurrentScopeRequiresPendingFilterAndRetryableCurrentScopeItem'` failed before implementation because current-scope XMP retry only scanned the loaded thumbnail window, then passed after moving retry lookup to a bounded catalog-query scope.
...
- Last broad unit verification: `swift test` passed after expanding current-scope XMP retry beyond the loaded window, with 914 tests, 1 skipped, and 0 failures. Earlier `swift test` passed after adding removable Smart Collection builder rules, with 912 tests, 1 skipped, and 0 failures. Earlier `swift test` passed after bounding current-scope evaluation queueing, with 911 tests, 1 skipped, and 0 failures.
- Current slice app workflow verification: No foreground app workflow run was performed for the XMP current-scope retry expansion because Jesse asked to minimize focus-stealing UI automation while actively using the machine; the change is covered by app-model/core/worker tests and full `swift test`. Earlier this turn, `./script/verify_headless_workflows.sh` passed on 2026-07-06, rebuilding the app/helper and running non-focus-stealing metadata-write, import-preview-drain, source availability, offline reconnect, preview render, local HTTP model smoke, and worker recovery checks.
...
- Current slice: made current-scope XMP pending retries scan the bounded catalog query scope instead of only the loaded thumbnail window, so retryable pending sidecars outside the first page can be queued.
