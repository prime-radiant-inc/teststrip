...
- Snapshot commit: `3ec8a1a Cancel individual background work rows`
- Product posture: foundation/dev build moving toward usable alpha, not yet a polished photo app.
- Last focused unit verification: `swift test --filter 'AppModelTests/testCancellingQueuedEvaluationWorkPreservesRunningPreviewAndDoesNotCancelAll|AppModelTests/testCancellingRunningEvaluationRestartsWorkerAndStartsNextQueuedWork|AppModelTests/testCanCancelBackgroundWorkActivityRequiresActiveBackgroundItem'` passed after row-level background work cancellation.
- Last broad unit verification: `swift test` passed with 570 tests after row-level background work cancellation.
