...
- Current slice focused verification: first-run sidebar no longer renders dead `Work > Recent` / `Work > Starred` placeholder rows when no real work sessions exist, while real Recent Work and Starred Work rows still render from catalog-backed sessions. The empty-work tests failed while the placeholder section was visible, then passed after the empty branch was removed and the work-history gap remained only in the code ledger. Earlier focused verification passed for live offline-XMP enqueue guards, import-completion optional-review cleanup, local import stability, source-availability sidebar tone, inspector source-status copy, culling-stack visual-similarity vectors, multi-label recognition, source-root unavailable counts, culling-stack placeholder cleanup, People dead-UI cleanup, OCR caption suggestions, work-session query scopes, plain selection batch collapse, import completion skipped-file review, import-issue plumbing, latest-import flagged review, People face-review, Search/parser quoted/import parsing, and recognition retry/gates.
