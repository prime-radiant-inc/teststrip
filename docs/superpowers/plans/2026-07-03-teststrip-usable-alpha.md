...
- Snapshot commit: `288f66a Handle unavailable originals during preview generation`
- Product posture: foundation/dev build moving toward usable alpha, not yet a polished photo app.
- Last focused unit verification: `swift test --filter WorkerCommandExecutorTests/testGeneratePreviewCommand` and `swift test --filter AppModelTests/testWorkerPreviewFailureRefreshesLoadedSourceAvailability` passed after the preview source-availability slice.
- Last broad unit verification: `swift test` passed with 522 tests after the preview source-availability slice.
...
- Live-mockup placeholders can be tagged in code with `LiveMockupPlaceholder`, including stable ids, intended behavior, and current fallback notes. The current registry tracks top chrome, People navigation, People face actions, agentic search, search refine, smart collections builder, import-complete summary, culling assist, culling filmstrip, stack cull, survey compare, and empty work history.
