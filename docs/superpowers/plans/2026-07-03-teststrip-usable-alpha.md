...
- Current slice: expose recorded catalog source roots as selectable Sources sidebar rows so NAS/removable/cloud source identity remains visible even when every original is currently online, and selecting a root reuses folder-prefix browsing instead of adding a new query path.
