...
- Snapshot commit: `09eab92 Add Compare Survey live mockup`
- Product posture: foundation/dev build moving toward usable alpha, not yet a polished photo app.
- Last focused unit verification: `swift test --filter CompareSurveyPresentationTests`, `swift test --filter AppModelTests/testCompare`, and `swift test --filter AppModelTests/testVisibleComparePreviews` passed after the Compare Survey live-mockup route slice.
- Last broad unit verification: `swift test` passed with 479 tests after the Compare Survey live-mockup route slice.
...
- Compare now uses a survey-style live mockup instead of a flat adaptive grid: the selected frame becomes the primary candidate, alternates stay selectable, preview requests still use cached progressive compare behavior, and Pick/Reject/Loupe actions write through existing metadata/navigation paths.
...
- Import UX is improved but not complete. The app now shows visible active-import feedback, post-import preview continuation, an Import Path plan, tested duplicate-import guards, and a compact import-complete action summary, but richer phase labeling, permission/security-scope failures, and card-source staging still need work.
