...
- Current slice focused verification: `swift test --filter 'PeoplePresentationTests|AppModelTests/testLoadExposesCatalogPeople|AppModelTests/testConfirmSelectedAssetAsPersonPersistsNamedGroup|AppModelTests/testConfirmSelectedBatchAsPersonUsesBatchInsteadOfPrimarySelection|AppModelTests/testMergePersonPersistsAndRefreshesCatalogPeople|AppModelTests/testDismissSelectedFaceReviewAssetsPersistsAndRefreshesReviewQueue|CatalogDatabaseTests/testCatalogsPeople|CatalogDatabaseTests/testMergesPeopleWithoutDuplicatingAssignments|CatalogDatabaseTests/testDismissingFaceAssetRemovesPersonAssignments|CatalogDatabaseTests/testDismissingFaceAssetRemovesItFromFaceReviewQueries'` passed while reconciling the stale People data-model checklist with the already-built confirmed-person catalog behavior.
...
- Current slice app workflow verification: No additional app workflow run was performed for the People checklist reconciliation because it is plan/status-only and backed by focused model/repository/presentation tests for existing behavior; Jesse asked to minimize focus-stealing automation. Recent app smoke verification still launched an isolated catalog with `./script/build_and_run.sh --verify-smoke` and captured a nonblank Library window at `/tmp/teststrip-latest-ui-smoke.png`; route-level Computer Use inspection remained blocked by desktop window detection.
