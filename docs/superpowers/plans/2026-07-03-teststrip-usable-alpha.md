...
- Snapshot commit: `5a89ce1 Add agentic review filters`
- Product posture: foundation/dev build moving toward usable alpha, not yet a polished photo app.
- Last focused unit verification: `swift test --filter AppModelTests/testLoadExposesReviewQueuesAndSelectingQueueAppliesFilter`, `swift test --filter AppModelTests/testLikelyIssuesFilterNamesSavedSearchScope`, `swift test --filter SearchSetTests`, and `swift test --filter CatalogDatabaseTests` passed after adding agentic review filters.
- Last broad unit verification: `swift test` passed with 577 tests after adding agentic review filters.
...
- `5a89ce1`: added catalog-backed Review queues for Faces Found, OCR Found, and Likely Issues, with sidebar counts, navigation, active-filter chips, and an explicit note that provider failures still need a durable per-asset failure contract.
