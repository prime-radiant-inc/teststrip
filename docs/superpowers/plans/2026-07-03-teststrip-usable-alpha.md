...
- Current slice focused verification: `swift test --filter 'EvaluationProviderTests|InspectorViewTests/testEvaluationSignalsGroupIntoPhotographerFacingSections'`, `swift test --filter 'EvaluationProviderTests|InspectorViewTests|LibrarySearchIntentTests|CopilotPresentationTests|CullingAssistPresentationTests|CompareSurveyPresentationTests'`, and `swift test --filter 'EvaluationProviderTests|WorkerCommandExecutorTests/testRuntimeConfigurationRegistersLocalImageMetricsProvider'` passed after adding framing as a model-provided signal kind and a local provisional motion-blur/softness signal.
...
- Last broad unit verification: `swift test` passed with 843 tests, 1 skipped, and 0 failures after adding framing and provisional motion-blur/softness signals.
- Current slice app workflow verification: No additional app workflow run was performed for framing and provisional motion-blur/softness because it changes worker/core/presentation behavior and is covered by focused provider, worker, inspector, search, Copilot, compare/culling presentation tests plus full `swift test`; Jesse asked to minimize focus-stealing automation. Recent app smoke verification still launched an isolated catalog with `./script/build_and_run.sh --verify-smoke` and captured a nonblank Library window at `/tmp/teststrip-latest-ui-smoke.png`; route-level Computer Use inspection remained blocked by desktop window detection.
...
- Current slice: added current-scope provisional keyword review so object-label suggestions can be surfaced from the full active query or saved set beyond the loaded thumbnail page, then accepted through catalog-first/XMP writeback only for matching assets.
...
- [x] Add first-class framing signals for model providers and a local provisional motion-blur/softness signal from cached-preview edge detail.
