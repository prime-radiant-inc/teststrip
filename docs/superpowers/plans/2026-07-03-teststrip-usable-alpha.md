...
- Current slice focused verification: `bash script/test_local_http_model_smoke_verifier_metrics.sh`, `script/verify_local_http_model_smoke.sh 5 3 1`, and `swift test --filter 'EvaluationProviderTests|BenchmarkCommandTests|WorkerCommandExecutorTests/testRuntimeConfigurationRegistersLocalHTTPModelProviderWhenConfigured'` passed after adding a repeatable local OpenAI-compatible HTTP stub smoke gate for the model-provider path.
...
- Last broad unit verification: `swift test` passed with 862 tests, 1 skipped, and 0 failures inside `script/verify_headless_workflows.sh` after adding the local HTTP model smoke verifier.
- Current slice app workflow verification: `script/verify_headless_workflows.sh` passed after running full tests, sandboxed app/helper build, metadata writeback verification with `matching_sidecar_metadata=100`, import-preview drain, source availability, preview render, local HTTP model smoke with `signals=3`, `vector_signals=1`, `visual_similarity_vector=1`, and worker recovery smoke. No additional UI launch was needed for this non-UI verifier slice.
