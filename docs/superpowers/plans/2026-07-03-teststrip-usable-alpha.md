...
- Current slice focused verification: the multi-label recognition tests failed before `EvaluationValue.labels` existed; focused EvaluationProvider/AppModel/CatalogDatabase tests passed after Apple Vision/local HTTP object outputs could preserve multiple labels in one signal, suggestions read both single-label and multi-label values, and catalog search matched stored multi-label values. Earlier focused verification passed for source-root unavailable counts, culling-stack placeholder cleanup, People dead-UI cleanup, OCR caption suggestions, work-session query scopes, plain selection batch collapse, import completion skipped-file review, import-issue plumbing, latest-import flagged review, People face-review, Search/parser quoted/import parsing, and recognition retry/gates.
...
- Last broad unit verification: `swift test` passed after the multi-label recognition slice, with 955 tests, 5 skipped, and 0 failures. Earlier full-suite checks in this run passed with 952, 951, 947, 945, 944, 943, 939, 937, 934, 931, 929, and 926 tests after the source-root unavailable-count slice, culling-stack placeholder cleanup, People dead-UI cleanup, OCR caption suggestion slice, work-session query-scope slice, plain-selection batch collapse, import issue review sheet, skipped-file issue plumbing, latest-import flagged review, People face-review, Search parser, recognition retry/gate, card-import smoke, card-import route, and culling-feedback slices.
- Current slice app workflow verification: `./script/verify_headless_workflows.sh` passed on 2026-07-06 at 06:50 with exit code 0. It ran the full Swift test suite (`955` tests, `5` skipped, `0` failures), rebuilt and signed the sandboxed app/helper, and ran non-focus metadata write, card import smoke, import preview drain, source availability, offline reconnect, preview render, local HTTP model smoke, real corpus smoke, and worker recovery checks. Earlier People dead-UI cleanup verification launched Teststrip with `/usr/bin/open -g --env TESTSTRIP_APPLICATION_SUPPORT_DIRECTORY=... -n dist/Teststrip.app`; process launch, normal restoration logs, and an onscreen `All Photographs` window were verified through a Swift/CoreGraphics window check. No foreground NSOpenPanel smoke was run because Jesse asked to minimize focus-stealing automation while using the machine.
