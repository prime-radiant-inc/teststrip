...
- Current slice focused verification: `swift test --filter PeoplePresentationTests` passed after removing visible disabled People future-action buttons, and `swift test --filter 'PeoplePresentationTests|LiveMockupPlaceholderTests'` passed with 34 tests after confirming the deferred face-work ledger still tracks automatic clustering, split, and face-box naming. Earlier focused verification passed for OCR caption suggestions, work-session query scopes, plain selection batch collapse, import completion skipped-file review, import-issue plumbing, latest-import flagged review, People face-review, Search/parser quoted/import parsing, and recognition retry/gates.
...
- Last broad unit verification: `swift test` passed after the People dead-UI cleanup slice, with 951 tests, 5 skipped, and 0 failures. Earlier full-suite checks in this run passed with 951, 947, 945, 944, 943, 939, 937, 934, 931, 929, and 926 tests after the OCR caption suggestion slice, work-session query-scope slice, plain-selection batch collapse, import issue review sheet, skipped-file issue plumbing, latest-import flagged review, People face-review, Search parser, recognition retry/gate, card-import smoke, card-import route, and culling-feedback slices.
- Current slice app workflow verification: `./script/verify_headless_workflows.sh` passed on 2026-07-06 after wiring the ignored real-photo corpus smoke into the default headless chain. It ran the full Swift test suite, rebuilt the sandboxed app/helper, and ran non-focus metadata write, card import smoke, import preview drain, source availability, offline reconnect, preview render, local HTTP model smoke, real corpus smoke, and worker recovery checks. Earlier People dead-UI cleanup verification used `./script/build_and_run.sh --build`, seeded a disposable smoke catalog, and launched Teststrip with `/usr/bin/open -g --env TESTSTRIP_APPLICATION_SUPPORT_DIRECTORY=... -n dist/Teststrip.app`; process launch, normal restoration logs, and an onscreen `All Photographs` window were verified through a Swift/CoreGraphics window check. No foreground NSOpenPanel smoke was run because Jesse asked to minimize focus-stealing automation while using the machine.
