...
- Current slice focused verification: `swift test --filter ImportFolderPathDraftTests/testCardPathPlanExplainsCopyBeforeCataloging --filter ImportFolderPathDraftTests/testValidCardPathsBuildCardConfirmationDraft --filter ImportFolderPathDraftTests/testInvalidCardDestinationKeepsDraftErrorForSheet` failed first because `ImportCardPathDraft` did not exist, then passed after adding the typed card source/destination path draft. `swift test --filter ImportFolderPathDraftTests/testCardPathReviewPresentationRequiresSourceAndDestination --filter ImportFolderPathDraftTests/testCardPathReviewPresentationShowsReviewingState` then failed first because the card review presentation did not exist, and passed after adding it. `swift test --filter ImportFolderPathDraftTests` also passed.
