...
- Search/sets/work sessions are partially built but not yet the full user-facing model. The Ask/search field now parses a narrow deterministic grammar into catalog predicates and related review filters, but clusters, work-session-derived sets, richer query editing, agent-generated refinements, and broader natural-language planning need more implementation.
