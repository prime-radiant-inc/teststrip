...
- Current slice focused verification: `swift test --filter AppModelTests/testSourceRootSidebarRowNamesUnavailableOriginals` failed before source-root sidebar rows named unavailable originals in their detail text; the focused source-root/source-availability AppModel test filter passed after the row detail started preserving the root path while appending the unavailable-original count. Earlier focused verification passed for culling-stack placeholder cleanup, People dead-UI cleanup, OCR caption suggestions, work-session query scopes, plain selection batch collapse, import completion skipped-file review, import-issue plumbing, latest-import flagged review, People face-review, Search/parser quoted/import parsing, and recognition retry/gates.
...
- [ ] **Recognition and provisional labels:** Local-first evaluation produces visible provenance-backed signals for faces, objects/content, OCR, focus, motion blur/softness, exposure, color/look, visual similarity, model-provided aesthetics/framing, and provider failures. Machine labels remain provisional until the user accepts them into metadata/XMP; selected object labels can become keywords, batch/current-scope object labels can become keywords, and selected OCR text can become a caption. Remaining gap: Apple Vision object classification currently collapses one provider result to a single object label; a small next slice should preserve multiple useful object labels from one provider result without auto-writing metadata.
