...
- Smart collections have a stronger live builder for current filters, concrete Add Rule preset rows, duplicate saved-set lifecycle, and a dynamic-to-frozen snapshot path, but freeform rule editing, destructive delete lifecycle, and agent-suggested rules are not complete.
