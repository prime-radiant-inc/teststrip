...
- Timeline is a first-class library route in the sidebar and top chrome. It uses catalog-backed capture-day summaries, groups days by month with catalog counts, shows a focused year/month/day scrubber for the loaded window, preserves loaded thumbnail activation, and lets day/month/year drill-down reuse existing captured-date predicates.
...
- Ratings, flags, labels, and keywords have app-model/catalog plumbing. The inspector can show object-label-backed keyword suggestions and accept them into catalog/XMP metadata; visible loaded assets can also aggregate object-label batch keyword suggestions, seed a batch metadata draft, and apply keywords/caption/creator/copyright to either visible assets or the current search/filter/set scope through the same catalog/XMP path. Current Scope metadata requires explicit confirmation before it targets the whole catalog.
...
- Library mockup parity is improving but incomplete. The overview grid now uses cataloged dimensions for true aspect-ratio cells, the filter rail is closer to the Studio mockup's Ask/search treatment, the inspector preview size is pinned and fixed above metadata scrolling, the inspector header/metadata controls have initial mockup-derived passes, the culling/loupe chrome now has verdict and filmstrip passes, imported-set stack culling uses real time-adjacent stack counts, persists detected stacks as hidden work-stack sets, and can navigate/accept/render stack membership beyond the initial loaded page, stack culling can act on a persisted-signal single-frame recommendation, People has a face-review strip, actionable review handoff, and named-people empty state, Search has advanced field parsing, a grouped refine rail, and existing-workflow suggested actions, Copilot has real primary actions and clearer empty review status rows over existing review/evaluation flows, Timeline has a catalog-backed year-density ribbon and focused month/day scrubber, Compare has a survey-style pass with metadata badges and active persisted-stack membership, Smart Collections has a split builder, visible/current-scope batch metadata review exists for portable fields with all-catalog confirmation, the sidebar has richer count/detail rows and real review-queue/saved-set/timeline counts, the top chrome has a first Studio-style pass, and major dead UI gaps plus all designer surfaces are tagged in code, but similarity-based near-duplicate detection, top-N focus stack ranking, persisted stack-decision history, multi-select batch metadata review, Timeline scroll-position syncing, agent-generated Search refinements, and deeper saved-query interactions still need visual passes against the design concept.
