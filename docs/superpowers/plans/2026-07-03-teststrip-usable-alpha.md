...
- Current slice focused verification: `swift test --filter 'AppModelTests/testSelectedSuggestedCaptionsComeFromOCRTextSignals|AppModelTests/testOCRTextRemainsProvisionalUntilAccepted|AppModelTests/testAcceptSuggestedCaptionForSelectedAssetWritesCatalogAndXmp|InspectorViewTests/testCaptionSuggestionPresentationNamesExplicitAcceptAction'` passed after making selected-photo OCR text signals visible as explicit caption suggestions that write catalog/XMP only after acceptance. Earlier focused verification passed for work-session query scopes, plain selection batch collapse, import completion skipped-file review, import-issue plumbing, latest-import flagged review, People face-review, Search/parser quoted/import parsing, and recognition retry/gates.
...
- Last broad unit verification: `swift test` passed after the OCR caption suggestion slice, with 951 tests, 5 skipped, and 0 failures. Earlier full-suite checks in this run passed with 947, 945, 944, 943, 939, 937, 934, 931, 929, and 926 tests after the work-session query-scope slice, plain-selection batch collapse, import issue review sheet, skipped-file issue plumbing, latest-import flagged review, People face-review, Search parser, recognition retry/gate, card-import smoke, card-import route, and culling-feedback slices.
- Current slice app workflow verification: For OCR caption suggestions, a disposable smoke catalog was seeded, the first smoke asset's existing caption was removed, an OCR text signal was inserted, `./script/build_and_run.sh --build` rebuilt the app/helper, and `/usr/bin/open -g --env TESTSTRIP_APPLICATION_SUPPORT_DIRECTORY=... -n dist/Teststrip.app` launched Teststrip against that isolated catalog without intentionally foregrounding it. Process launch, Teststrip logs, and an onscreen `All Photographs` window were verified, but visual confirmation is still not accepted: `script/capture_app_window.sh` found the Teststrip window and then `screencapture -l` failed with `could not create image from window`; direct `screencapture -l <windowID>` failed the same way, and read-only Accessibility exposed only app/menu text without foreground activation. Earlier `./script/verify_headless_workflows.sh` passed on 2026-07-06 after adding the card-import smoke to the chain. It rebuilt the sandboxed app/helper and ran non-focus metadata write, card import smoke, import preview drain, source availability, offline reconnect, preview render, local HTTP model smoke, and worker recovery checks. No foreground NSOpenPanel smoke was run because Jesse asked to minimize focus-stealing automation while using the machine.
