...
- Current slice focused verification: `swift test --filter 'AppModelTests/testRemovingSelectedDynamicSetRuleDetachesSetAndPreservesRemainingScope|AppModelTests/testRemovingSelectedDynamicSetRulePreservesRemainingLikelyIssueScope|AppModelTests/testRemovingSelectedDynamicSetRulePreservesRemainingCameraScopeWithSpaces|CullingCommandMenuPresentationTests'` passed after exposing stack culling menu commands and preserving detached dynamic-set predicates as structured filters.
- Last focused unit verification: `swift test --filter 'AppModelTests/testRemovingSelectedDynamicSetRulePreservesRemainingLikelyIssueScope|AppModelTests/testRemovingSelectedDynamicSetRulePreservesRemainingCameraScopeWithSpaces|CullingCommandMenuPresentationTests|CullingKeyCaptureTests|AppModelTests/testCullingShortcut'` passed with 24 tests after making stack culling commands discoverable and fixing detached dynamic-set rule preservation. Earlier focused verification includes Search refinement removal, removable Library filter chips, preview queue thumbnail state, inspector sizing, offline reconnect smoke, empty import summaries, source reconnect errors, import cancel affordance, Copilot scope actions, culling stack guidance, latest-import evaluation handoff, People review-card lock-state, generated Search refinements, People scan action, saved-set lifecycle, RAW catalog-only import, batch metadata/XMP, culling, and import-path slices listed below.
- Last broad unit verification: `swift test` passed after exposing stack culling commands and preserving detached filters, with 907 tests, 1 skipped, and 0 failures. Earlier `swift test` passed after making Search active refinements removable, with 903 tests, 1 skipped, and 0 failures. Earlier `swift test` passed after making active Library filter chips removable, with 902 tests, 1 skipped, and 0 failures.
- Current slice app workflow verification: No foreground app workflow run was performed for the culling command menu change because Jesse asked to minimize focus-stealing UI automation while actively using the machine; the change is covered by menu presentation tests, shortcut/key-capture tests, model behavior tests, SwiftUI compilation, and full `swift test`.
