...
- Current slice focused verification: `swift test --filter 'ImportFolderPathDraftTests/testMatchingCardSourceAndDestinationKeepsDraftErrorForSheet|ImportFolderPathDraftTests/testCardSourceInsideDestinationKeepsDraftErrorForSheet'` failed first because the typed card path draft did not throw or keep sheet errors for unsafe source/destination relationships. `swift test --filter ImportFolderPathDraftTests` passed after the draft reused the existing card destination preflight before constructing confirmation drafts.
