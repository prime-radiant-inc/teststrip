...
- Current slice focused verification: `swift test --filter 'CatalogDatabaseTests/testAssignedPersonAssetRemovesItFromFaceReviewQueries|AppModelTests/testConfirmSelectedPersonRemovesAssignedAssetsFromFaceReviewQueue'` and the broader People/person/face-review group passed after making named people leave unnamed face-review queues while preserving raw face signals. Immediately before that, focused Search/parser verification passed for quoted field parsing, import-batch parsing, and quoted round-trip serialization, and focused recognition verification passed for cached-preview evaluation gates plus retrying failed provider work.
...
- Last broad unit verification: `swift test` passed after the named-person face-review slice, with 939 tests, 5 skipped, and 0 failures. Earlier full-suite checks in this run passed with 937, 934, 931, 929, and 926 tests after the Search parser, recognition retry/gate, card-import smoke, card-import route, and culling-feedback slices.
