...
- Current slice focused verification: `swift test --filter AppModelTests/testVisibleWorkActivitiesDoNotClaimUndispatchedWorkerItemsAreRunning` passed after making Activity status avoid claiming undispatched worker commands are already running.
- Last focused unit verification: `swift test --filter 'AppModelTests/testVisibleWorkActivities|AppModelTests/testBackgroundWork|AppModelTests/testBackgroundControlsForwardToWorkerSupervisor|AppModelTests/testCanCancelBackgroundWorkActivity|AppModelTests/testCancellingQueuedEvaluationWorkPreservesRunningPreviewAndDoesNotCancelAll|AppModelTests/testCancellingRunningLocalHTTPModelEvaluationRestartsWorkerAndStartsNextQueuedWork|WorkerSupervisorTests'` passed with 34 tests after correcting user-facing worker activity status. Earlier focused verification includes latest-import preview status scoping, queued import progress copy, culling command menu presentation, Search refinement removal, removable Library filter chips, preview queue thumbnail state, inspector sizing, offline reconnect smoke, empty import summaries, source reconnect errors, import cancel affordance, Copilot scope actions, culling stack guidance, latest-import evaluation handoff, People review-card lock-state, generated Search refinements, People scan action, saved-set lifecycle, RAW catalog-only import, batch metadata/XMP, culling, and import-path slices listed below.
- Last broad unit verification: `swift test` passed after correcting user-facing worker activity status, with 909 tests, 1 skipped, and 0 failures. Earlier `swift test` passed after scoping latest-import preview status, with 908 tests, 1 skipped, and 0 failures. Earlier `swift test` passed after clarifying queued import progress copy, with 907 tests, 1 skipped, and 0 failures.
- Current slice app workflow verification: No foreground app workflow run was performed for the worker activity status correction because Jesse asked to minimize focus-stealing UI automation while actively using the machine; the change is covered by app-model tests, SwiftUI compilation, WorkerSupervisor tests, and full `swift test`.
...
- Current slice: made Activity/visible work status stop overclaiming undispatched worker commands as running when the queue is allowed to mark more work running than the single worker process has actually dispatched.
