...
- Current slice focused verification: the culling-stack rail visual-similarity test failed before the rail passed persisted `.visualSimilarity` vectors into `AssetStackBuilder`; focused `CullingStackRailPresentationTests` and `AppModelTests/testCompareAssetsUseVisualSimilaritySignalsAcrossFoldersAndCaptureTimes` passed after the rail used the highest-confidence vector signal per asset. Earlier focused verification passed for multi-label recognition, source-root unavailable counts, culling-stack placeholder cleanup, People dead-UI cleanup, OCR caption suggestions, work-session query scopes, plain selection batch collapse, import completion skipped-file review, import-issue plumbing, latest-import flagged review, People face-review, Search/parser quoted/import parsing, and recognition retry/gates.
