...
- Current slice: add a non-focus-stealing headless card-import smoke benchmark/verifier that copies from a simulated card source to a destination root, generates cached previews, verifies catalog paths under the destination, and confirms source originals remain unchanged.
