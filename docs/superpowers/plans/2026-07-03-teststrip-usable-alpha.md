...
- Current slice app workflow verification: For People dead-UI cleanup, `./script/build_and_run.sh --build` rebuilt and signed the app/helper, a disposable smoke catalog was seeded, and `/usr/bin/open -g --env TESTSTRIP_APPLICATION_SUPPORT_DIRECTORY=... -n dist/Teststrip.app` launched Teststrip against that isolated catalog without intentionally foregrounding it. Process launch, normal restoration logs, and an onscreen `All Photographs` window were verified through a Swift/CoreGraphics window check. Earlier `./script/verify_headless_workflows.sh` passed on 2026-07-06 after adding the card-import smoke to the chain. It rebuilt the sandboxed app/helper and ran non-focus metadata write, card import smoke, import preview drain, source availability, offline reconnect, preview render, local HTTP model smoke, and worker recovery checks. No foreground NSOpenPanel smoke was run because Jesse asked to minimize focus-stealing automation while using the machine.
