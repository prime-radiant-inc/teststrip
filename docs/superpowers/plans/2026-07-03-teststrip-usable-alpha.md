...
- Current slice focused verification: Ordinary grid click selection and batch selection no longer force the hidden culling key-capture view to become first responder, while loupe opening and accessibility selection still request culling focus. Earlier focused verification passed for active unknown-total import scan counts, count-aware catalog-scale thresholds, import progress reassurance copy, optional real LM Studio/Ollama-style provider smoke, import source-root/sidebar refresh, Search current-scope culling, empty/all-skipped import payoff cleanup, focus-signal Search/Smart Collection refinements, loaded-stack culling progress, duplicate/existing-only import payoff copy, typed card-import workflow automation, Search plain fallback, Compare/Survey culling-session progress, RAW fixture verification, typed card-import path conflicts, worker-backed selected metadata XMP coalescing, Library sorting, grid keyword badges, selected provider failure retry guarding, selected provider failure surfacing, empty Review sidebar cleanup, review queue phrase parsing, empty Folders sidebar cleanup, empty Work sidebar cleanup, live offline-XMP enqueue guards, import-completion optional-review cleanup, local import stability, source-availability sidebar tone, inspector source-status copy, culling-stack visual-similarity vectors, multi-label recognition, source-root unavailable counts, culling-stack placeholder cleanup, People dead-UI cleanup, OCR caption suggestions, work-session query scopes, plain selection batch collapse, import completion skipped-file review, import-issue plumbing, latest-import flagged review, People face-review, Search/parser quoted/import parsing, and recognition retry/gates.
- Last focused unit verification: `swift test --filter LibraryGridLayoutTests` passed after adding the grid activation focus policy. Earlier focused verification includes import progress presentation tests, catalog-scale verifier metric tests, import progress reassurance copy, optional real local HTTP model smoke, import source-root/sidebar refresh, Search current-scope culling, empty/all-skipped import payoff cleanup, focus-signal refinements, loaded-stack culling progress, duplicate-import matched-set copy, typed card-import workflow verifier, Search plain-fallback, Compare culling-session progress, RAW fixture verifier, typed card path confirmation, selected metadata XMP coalescing, Library sort, grid keyword badge, provider retry enablement, selected provider failure, empty Review sidebar cleanup, review queue search phrase, empty Folders sidebar cleanup, current-scope XMP retry, removable Smart Collection builder rules, current-scope evaluation queueing, worker activity status, latest-import preview status scoping, queued import progress copy, culling command menu presentation, Search refinement removal, removable Library filter chips, preview queue thumbnail state, inspector sizing, offline reconnect smoke, empty import summaries, source reconnect errors, import cancel affordance, Copilot scope actions, culling stack guidance, latest-import evaluation handoff, People review-card lock-state, generated Search refinements, People scan action, saved-set lifecycle, RAW catalog-only import, batch metadata/XMP, culling, and import-path slices listed below.
- Last broad unit verification: `swift test` passed after the grid activation focus slice, with 989 tests, 5 skipped, and 0 failures. Earlier full-suite checks in this run passed with 988, 987, 984, 983, 980, 980, 980, 979, 979, 978, 977, 976, 975, 973, 972, 972, 971, 970, 966, 965, 964, 961, 959, 958, 958, 957, 955, 957, 956, 955, 952, 951, 947, 945, 944, 943, 939, 937, 934, 931, 929, and 926 tests after the active import scan-count, catalog-scale threshold, import source-root/sidebar guard, optional real local HTTP model smoke wrapper, Search current-scope culling action, empty-import payoff cleanup, focus-signal Search/Smart Collection slice, loaded-stack culling progress, duplicate-import matched-set copy, typed card-import workflow verifier, Search plain-fallback, Compare culling-session progress, RAW fixture verifier, typed card path confirmation, selected metadata XMP coalescing, Library sort, grid keyword badge, provider retry enablement, selected provider failure, empty Review sidebar cleanup, review queue search phrase, empty Folders sidebar cleanup, empty Work sidebar cleanup, offline live-XMP enqueue guard, import-completion optional-review cleanup, local import stability, source-availability sidebar tone, inspector source-status copy, culling-stack visual-similarity, multi-label recognition, source-root unavailable-count, culling-stack placeholder cleanup, People dead-UI cleanup, OCR caption suggestion, work-session query-scope, plain-selection batch collapse, import issue review sheet, skipped-file issue plumbing, latest-import flagged review, People face-review, Search parser, recognition retry/gate, card-import smoke, card-import route, and culling-feedback slices.
- Current slice app workflow verification: For the grid activation focus slice, `./script/build_and_run.sh --build` rebuilt and signed `dist/Teststrip.app` plus `TeststripWorker` without launching foreground UI. Earlier active import scan-count verification also rebuilt and signed without launching foreground UI. Earlier catalog-scale threshold verification passed `script/verify_catalog_scale.sh 100000` with the 0.2s default threshold, `script/verify_catalog_scale.sh 500000` with the 0.65s default threshold, and `script/verify_catalog_scale.sh 1000000` with the 1.5s default threshold. Earlier import-confidence banner verification rebuilt and signed the app/helper without launching UI. Earlier optional real local HTTP model smoke verification skipped cleanly without endpoint configuration and rebuilt and signed the app/helper without launching UI. Earlier import source-root/sidebar guard verification rebuilt and signed the app/helper without launching UI. Earlier Search current-scope culling action verification rebuilt and signed the app/helper without launching UI. Earlier empty-import payoff cleanup verification rebuilt and signed the app/helper without launching UI. Earlier focus-signal Search/Smart Collection verification rebuilt and signed the app/helper without launching UI. Earlier loaded-stack culling progress verification rebuilt and signed the app/helper without launching UI. Earlier duplicate-import matched-set copy verification rebuilt and signed the app/helper without launching UI. Earlier typed card-import workflow verifier verification rebuilt and signed the app/helper without launching UI, and `bash script/test_verify_app_workflows.sh` proved the workflow gate launches with `TESTSTRIP_CARD_IMPORT_ROUTE=typed-path` and invokes `verify_card_import_path.sh` after `verify_import_path.sh`. The foreground `verify_card_import_path.sh` app run is wired but not yet executed against the real UI because Jesse asked to minimize focus-stealing automation while using the machine. Earlier Search plain-fallback verification rebuilt and signed the app/helper without launching UI. Earlier Compare culling-session progress verification rebuilt and signed the app/helper without launching UI. Earlier RAW fixture verifier verification rebuilt and signed the app/helper without launching UI. Earlier typed card path confirmation verification rebuilt and signed the app/helper without launching UI. Earlier selected metadata XMP coalescing verification rebuilt and signed the app/helper without launching UI. Earlier Library sort verification rebuilt and signed the app/helper, and `/usr/bin/open -g -n dist/Teststrip.app` launched the app as process `Teststrip` without foregrounding. A read-only Computer Use snapshot by app path failed with `cgWindowNotFound`, so no foreground UI inspection was forced while Jesse is using the machine. Earlier background `open -g` + CoreGraphics smoke proved Teststrip can create an on-screen isolated smoke window, but `screencapture -l` failed with `could not create image from window` for that background-launched window even after the window was present, so foreground visual inspection remains deferred while Jesse is using the machine. Earlier `./script/verify_headless_workflows.sh` passed on 2026-07-06 at 06:50 with exit code 0. It ran the full Swift test suite (`955` tests, `5` skipped, `0` failures), rebuilt and signed the sandboxed app/helper, and ran non-focus metadata write, card import, preview drain, source availability, offline reconnect, preview render, local HTTP model smoke, real corpus smoke, and worker recovery checks. Earlier People dead-UI cleanup launched Teststrip in the background and verified process/window state through CoreGraphics. No foreground NSOpenPanel smoke was run because Jesse asked to minimize focus-stealing automation while using the machine.
...
- `9aeb808`: stopped ordinary grid click and batch selection from stealing focus into the hidden culling key-capture view while preserving explicit focus for loupe and accessibility activation.
- `25ac422`: made active unknown-total imports show `Counting photos` and `<n> found` in the progress banner so large source scans do not look blank while the final count is still unknown.
- `1888620`: added measured count-aware default thresholds for the catalog-scale verifier at 100k, 500k, and 1M synthetic assets while preserving explicit timing overrides.
- `372f0ff`: made the import progress banner more explicit by adding reassurance copy for queued, active, and preview-building phases so the flow does not look stalled while thumbnails/previews catch up.
- `1a6b33b`: added `seed-real-corpus-catalog` plus `build_and_run.sh --verify-real-corpus`, creating a normal isolated app catalog from the ignored local real-photo corpus and launching it in the background without committing sample files.
- `48de26b`: changed People's Apple Vision scan from visible-page scope to the current catalog/search scope, reusing the bounded current-scope evaluator so cached assets beyond the loaded page can be queued.
- `7ab1058`: added catalog-backed Search work-history matches so residual plain search text can surface prior work sessions in the Search rail and reopen them through the existing `session:<id>` scope.
- `14a9100`: added an optional real local HTTP model smoke wrapper for LM Studio/Ollama-style endpoints, wired it into the headless gate as a skip-safe provider check, and tested both skip and configured-command paths.
- `39895b4`: added import completion regression coverage proving folder and card imports immediately refresh the active source-root model and Sources sidebar rows after persisting bookmark data, without requiring app relaunch.
- `0f58e1f`: added a Search suggested action that starts culling for the current result scope through the existing culling-session popover, making saved/search scopes a visible arbitrary-set culling entrypoint.
- `52c008d`: hid empty/all-skipped import payoff action cards that would otherwise offer disabled Open/Review/Evaluate/Cull actions for an imported set that does not exist, while preserving real skipped-file issue review.
- `0627c18`: added focus provider signals as first-class Search generated refinements and Smart Collection rule presets, filtering by existing catalog evaluation summaries without auto-writing metadata or overclaiming sharpness.
- `a182cce`: made loaded-stack culling decisions refresh active culling work-session progress and the Picks output set after accepting a detected stack frame, matching persisted stack and Compare decision behavior.
- `c5090b5`: made duplicate/existing-only import payoff actions say matched set/already-cataloged photos instead of imported set/imported frames, so re-import recovery does not imply new files were added.
- `96512f2`: added an automation-only typed card-import route, forwarded it through the app launch script, wired the app workflow gate to run a card-import path verifier after folder Import Path, and added a shell contract proving the workflow order and launch environment.
- `f46a443`: added a visible plain-search fallback row to the Search Teststrip Reads rail for unparsed Ask text, preserving removable parsed filters while making the current deterministic parser limit honest.
- `38ab4ab`: made Compare/Survey pick-reject actions refresh active culling work-session progress and the session Picks output set after applying batch flags, so arbitrary-set culling stays coherent when decisions happen from Compare rather than only Loupe.
- `f120416`: added an ignored-local RAW fixture verifier to the headless alpha gate, exercising DNG, Canon CR2, and Fuji RAF through real ImageIO metadata/dimension reads without committing photo files while leaving CRW/X3F explicitly missing.
- `8bc184c`: made typed card-import source/destination relationship conflicts return the normal blocked confirmation draft with visible reason and Start disabled instead of throwing before the review sheet can open.
- `9d893aa`: coalesced stale queued worker-backed XMP write jobs for rapid selected-photo metadata edits, keeping only the latest catalog generation runnable while preserving immediate catalog updates and leaving running sync work alone.
- `c986cfe`: added catalog-backed Library sorting by import order, capture time newest/oldest, and filename, with current filtered-scope reload and a compact Library filter-bar picker matching the designer's sort affordance.
- `f75a9ec`: removed the empty first-run Work sidebar placeholder rows so Recent Work and Starred Work appear only when real catalog-backed sessions exist, with the remaining work-history gap tracked in the live-mockup ledger instead of visible dead UI.
- `707b326`: removed the empty first-run Folders sidebar placeholder row so folder navigation appears only when real catalog-backed folders exist, with the remaining folder-navigation gap tracked in the live-mockup ledger instead of visible dead UI.
- `e90c077`: added a compact keyword-count badge to grid thumbnail metadata overlays so keyword edits receive immediate browsing-surface acknowledgement alongside rating, flag, and color-label badges.
- `1c1bbe4`: disabled selected-provider retry actions unless the selected asset has a matching durable failure, cached preview, worker supervisor, and no active retry already in flight.
- `d774d8f`: surfaced selected-photo provider failures in the inspector with provider/message detail and a retry action that reuses the existing evaluation worker request path.
- `60b1da4`: removed empty Review sidebar queues so the Review section appears only when catalog-backed review work exists, while preserving populated queues such as Picks and Needs Evaluation.
- `7b05798`: taught deterministic Search and typed Smart Collection rules to parse existing review queue phrases such as Faces Found, OCR Found, Likely Issues, Provider Failures, and XMP pending/conflict into structured catalog predicates instead of plain residual search text.
- `52ca0da`: kept live selected and batch metadata edits catalog-first for offline/unwritable originals by recording pending XMP rows without dispatching worker sync until the sidecar path is retryable, while preserving immediate worker dispatch for online writable originals.
- `82b865a`: hid unavailable optional import-completion review actions for flagged frames, keyword suggestions, and face review so the payoff panel only shows those follow-ups when there are actual candidates, while preserving core import, Evaluate import, issue review, and stack-cull actions.
- `fc21220`: kept local fallback import progress anchored on the first cataloged asset while later progress updates counts/detail without jumping selection or expanding the loaded grid page mid-import.
- `559c552`: toned `Offline Originals` source sidebar rows as warning rather than destructive, preserving destructive severity for missing/moved originals while keeping stale originals warning-level.
- `e691b56`: made the selected-asset inspector reuse the shared source-availability presentation, so offline originals now read as cached-preview-only workflow state instead of raw enum status in both visible and accessibility copy.
- `3529558`: made the culling stack rail honor persisted visual-similarity vectors from Apple Vision/local model signals when building loaded fallback stacks, so similarity-based near-duplicate stacks now surface in loupe culling with distance/threshold rationale instead of only in Compare/import paths.
- `bad0deb`: preserved multiple object/content labels from one Apple Vision or local HTTP provider result in a single evaluation signal, kept those labels provisional, and made suggestions/search consume both single-label and multi-label values.
- `f28457a`: made source-root sidebar rows name how many originals under that root are unavailable while keeping the total asset count badge and root-filter behavior unchanged.
- `c7e7153`: removed the disabled `Keep top 2` culling-stack placeholder when no ranking signals exist, leaving only real no-signal stack actions while preserving signal-backed recommended/top-ranked actions.
- `80f413a`: wired the ignored real-photo corpus smoke into `./script/verify_headless_workflows.sh` and added a shell contract test so realistic JPEG/DNG/RAF import coverage remains part of the repeatable alpha gate without committing sample images.
- `e61070f`: removed visible disabled People future-action buttons from the normal People route, replacing them with quiet status copy while keeping automatic clustering, split, and face-box naming tracked in the live-mockup ledger.
- `10056fd`: added selected-photo OCR text caption suggestions that remain provisional until explicit acceptance, then write catalog metadata and XMP sidecars through the existing selected-metadata path while leaving originals untouched.
- `ae84a8e`: made a normal asset selection clear any existing batch selection so rating, flagging, keywording, and save-set actions do not silently target an older command/shift batch after the user clicks a single photo.
- `cee3224`: made work-session rows, Recently Added, import-completion open/review flows, and active `session:`/`import:` filters browse query scopes rather than hidden output sets; dynamic culling inputs are snapshotted at session start so work history remains stable.
- `fb68dbf`: added a conditional import-completion `Review skipped files` action and local issue sheet so skipped source paths/messages are directly inspectable from the completion banner.
- `455e4f1`: persisted skipped-source import issue details in completed work sessions, carried them through worker import completion events/results, and surfaced them as import-completion issue metrics.
- `b63f367`: added a latest-import scoped `Review flagged` action that opens likely-issue assets from the import output set without writing flags, metadata, or XMP.
- `90c6462`: made manually named person assets leave unnamed face-review queues and face-signal review summaries immediately while keeping the underlying face evaluation signals intact.
- `4edb30a`: taught Search/Smart Collection rule text to keep quoted folder/keyword values with spaces together, parse `import:` batch predicates, and preserve quoted field values when removing parsed filter chips.
- `5b77999`: made selected/visible recognition actions require cached-preview candidates and allowed failed inactive provider work to be retried while still suppressing active duplicate evaluation jobs.
- `45cbf98`: added a non-focus-stealing headless card-import smoke benchmark/verifier that copies from a simulated card source to a destination root, generates cached previews, verifies catalog paths under the destination, confirms source originals remain unchanged, and runs in `./script/verify_headless_workflows.sh`.
- `94517fd`: routed the primary `Import Card` action through the user-granted folder panel and proved the sandbox-style worker-disabled card path copies into the destination root, generates previews, avoids worker commands, and persists the destination-root bookmark.
- `d89d0b0`: added culling shortcut feedback so keyboard metadata decisions remain visible after auto-advance, naming the changed frame and decision in the culling header.
- `46a684a`: added a real `Recently Added` Library row backed by the latest completed import output set, matching the Studio mockup's post-import browsing route without adding a parallel query path.
- `cc022be`: exposed recorded catalog source roots as selectable Sources sidebar rows so NAS/removable/cloud source identity remains visible even when every original is currently online, and selecting a root reuses folder-prefix browsing.
- `53d36a0`: strengthened opt-in RAW fixture coverage so available local DNG/RAF fixtures run ImageIO metadata/dimension reads while missing CRW/CR2/X3F fixtures skip independently.
...
- `06622ee`: made Activity/visible work status stop overclaiming undispatched worker commands as running when the queue is allowed to mark more work running than the single worker process has actually dispatched.
- `2a090be`: scoped latest-import preview completion status to the import output set so unrelated preview work no longer makes a finished import look like its previews are still building.
- `101573f`: clarified queued import progress detail so waiting imports name the managed background worker queue instead of looking like a stalled generic waiting state.
- `bb0dfc3`: exposed previous/next stack and accept-stack-selection in the macOS Culling menu through a tested menu presentation, and preserved detached dynamic-set rules as structured filters instead of lossy search text.
- `ac5ca0a`: made Search active refinement chips and rail rows removable while preserving related-filter and generated-refinement action behavior.
- `f679a85`: made active Library filter chips removable controls backed by model behavior for explicit filters, parsed Ask/search chips, selected saved-set scope, and selected dynamic-set rule detachment.
- `181d6d0`: showed real preview queue state on grid thumbnails so imported assets without cached previews read as `Preview queued`, `Building preview`, or `Preview issue` instead of looking like blank/broken cells.
- `a47d8cd`: pinned the right inspector column width to the selected-preview size plus padding so the preview no longer depends on a flexible minimum-width host column.
- `9aea345`: added a headless offline reconnect smoke verifier that proves a cataloged offline original can keep its cached grid preview readable, reconnect to a remounted root, move XMP sync state to the relocated sidecar path, and leave original/sidecar bytes unchanged.
- `f21943e`: exposed completed zero-photo imports through the import summary panel, including all-skipped imports, `No previews needed` copy, and disabled imported-set actions when no set exists.
- `d11540d`: rejected no-op source reconnects with specific wrong-old-root, missing-file, and fingerprint-mismatch errors, keeping the reconnect sheet open with inline error copy instead of reporting `Reconnected 0 sources`.
- `9bb4ac9`: clarified the import progress banner cancel affordance by naming the active import source in help and accessibility copy, keeping cancellation behavior unchanged.
- `ecb11bd`: exposed Save Dynamic Set and Freeze Snapshot actions from Copilot when the current read scope can be saved, reusing the existing Search popover flows instead of adding a second persistence path.
- `40e890b`: surfaced stack-level Keep recommended / Keep top-ranked guidance in the rapid-cull `TESTSTRIP READS` header, kept selected-frame reads as fallback, avoided duplicate stack presentation construction in SwiftUI render paths, and refreshed the live-mockup ledger.
- `760857b`: added an import-complete Evaluate import handoff gated by worker availability plus cached latest-import previews, queued recognition only for cached assets in the import output set, and scoped the face-review payoff count to latest-import assets instead of global Faces Found.
- `f812c19`: removed stale People review-card lock state so face and face-quality review cards present live handoffs, while only Auto cluster, Split person, and Face-box naming remain disabled future actions in the ledger.
- `107d1c0`: tightened the typed card import path sheet to reject unsafe source/destination relationships before constructing the confirmation sheet, reusing the existing card-copy preflight and keeping the error on the path sheet.
- `a00ec43`: added a typed card import path sheet with source and destination fields, review/progress/error state, existing card confirmation preflight, and routing into the existing copy-from-card import backend without relying on sequential native file panels.
...
- Selected pending XMP sync can be retried explicitly; direct retries write sidecars without touching original bytes, and worker-backed retries reuse the managed XMP queue across the bounded current catalog query instead of only the loaded thumbnail window.
...
- Reconnect no-op attempts now fail explicitly instead of reporting success: wrong old roots, missing relative files under the new root, and fingerprint mismatches each produce specific inline sheet errors while leaving catalog paths untouched.
- `./script/verify_offline_reconnect_smoke.sh` now proves the headless offline/reconnect invariant: cached grid preview remains readable before and after reconnect, the catalog asset becomes online under the remounted root, XMP sync state follows the sidecar path, and original/sidecar bytes remain unchanged.
...
- Recognition can be requested for selected, visible, compare, and current-scope cached assets; current-scope evaluation can enqueue a bounded batch of cached assets beyond the loaded grid page while skipping uncached assets.
- Selected-frame evaluation signals now feed a compact culling verdict presentation, and active stack recommendations can override that fallback so the rapid-cull header shows real `TESTSTRIP READS` state with supporting quality rationale or stack-level keeper guidance instead of a static placeholder.
...
- People is selectable from the sidebar and top chrome as a live mockup backed by real face-signal coverage. It now shows an unnamed face-review strip with live queue/scan status, a current-scope Apple Vision scan action for cached visible previews, and catalog-backed face review cards for face detections/quality signals; those cards route into the existing Faces Found and face-quality review targets. Users can name selected photos as a confirmed person, merge confirmed people, and dismiss selected photos from face-review queues while preserving the underlying evaluation signals. The sidebar labels this surface as `Face review`, and automatic clustering, split, and face-box-level naming remain disabled placeholders.
...
- The import-complete summary is an expanded partial payoff panel with real import count, preview status, Open/Cull actions, time-adjacent stack counts when detected, a gated Evaluate import action for cached latest-import previews, a latest-import keyword review action when suggestions exist, dismiss behavior, and a face-review follow-up whose count is scoped to the latest import.
...
- Import progress copy now distinguishes starting, scanning, cataloging, copying, and preview-building phases with tested presentation rules and visible counts where available, and the banner cancel affordance names the active import source when source detail is available.
- Completed imports show an expanded summary panel with the imported photo count, preview status/failure count, Open action for the imported output set, Cull action that starts a culling work session from that set, Cull stacks action that persists each detected time-adjacent stack as a hidden work-stack set and starts culling with the first stack set selected, Evaluate import for cached latest-import previews, keyword suggestion review through the batch metadata popover, import-scoped face review handoff when face signals exist, and dismiss behavior. Completed zero-photo imports now also show a terminal summary with `No photos imported`, `No previews needed`, and imported-set actions disabled instead of disappearing into status text.
- Folder and card import entrypoints refuse duplicate import submissions while an import is already running.
- Culling sessions now start and reopen in loupe view with a culling header, reviewed-progress bar, pick/reject counts, selected-frame `TESTSTRIP READS` verdict with compact supporting quality rationale, stack-level `TESTSTRIP READS` keeper guidance when persisted quality signals rank frames in the active stack, fixed-height bottom filmstrip, stable rating/label/flag command rail, visible frame position, persisted stack-set navigation/acceptance and rail presentation for import stack culls, visual-similarity vector stack grouping with distance/threshold rationale when Apple Vision or local-model signals exist, work-session progress/completion from decided catalog flags, reviewed/pick/reject detail in recent/starred work history, a session Picks output set once accepted frames exist, stale empty Picks output cleanup when picks are cleared, and concrete Keep recommended / Keep top 2 actions when persisted focus/quality signals rank frames in the current stack.
...
- Ratings, flags, labels, keywords, and captions have app-model/catalog plumbing. The inspector can show object-label-backed keyword suggestions and OCR-backed selected-photo caption suggestions, and accept them into catalog/XMP metadata only after an explicit user action; visible loaded assets and latest-import completions can also aggregate object-label batch keyword suggestions, seed a batch metadata draft, and apply keywords/caption/creator/copyright to command-selected loaded assets, visible assets, or the current search/filter/set scope through the same catalog/XMP path. Worker-backed batch metadata persists catalog edits, records pending XMP rows before enqueue, and queues sync work in one supervisor batch to avoid per-asset UI churn. Current Scope metadata requires explicit confirmation before it targets the whole catalog.
...
- Import UX is improved but not complete. The app now shows visible active-import feedback, phase labels, explicit reassurance copy for queued/active/preview phases, queued import progress that names the background worker queue, post-import preview continuation scoped to the imported output set, queued/building/issue thumbnail state for assets waiting on cached previews, durable skipped-file issue details on completed import work sessions, an Import Path plan, tested duplicate-import guards, runtime missing folder/card-source/card-destination errors, core card-destination policy guards, capped source previews, honest follow-up setup rows, a compact import-complete action summary including empty/all-skipped terminal results without no-op imported-set action cards, skipped-file issue metrics plus a skipped-file review sheet, and scoped review handoffs for likely issues, Compare, local evaluation, stack culling, face review, and keyword suggestions, and a sandbox-aware required security-scope policy for packaged dev runs. Primary Card Import now uses the user-granted folder panel route instead of the typed path sheet, the worker-disabled card path is covered for local copy, preview generation, source access release, no worker commands, and destination-root bookmark persistence, and `./script/verify_card_import_smoke.sh` provides a repeatable non-focus smoke for card copy, destination cataloging, cached previews, and unchanged source originals. Completed imports persist security-scoped bookmark data for catalog source roots when macOS provides it, model load restores access from stored bookmarks for the app session, recorded source roots stay visible as selectable Sources sidebar rows even when all originals are online, stale/failed bookmark restore is visible in diagnostics and the Sources sidebar as a repair-needed actionable reconnect row, source-root reconnect persists a fresh bookmark for the mounted root when macOS provides one, and no-op reconnect attempts now show explicit inline failures instead of looking successful. A restrained sandboxed UI import smoke still needs work.
- Clicking/selection needs one more accepted imported-photo verification pass. Direct grid clicks no longer recenter the thumbnail under the pointer, which was the likely root cause of the weird click feeling, policy tests cover pointer-vs-programmatic selection scroll behavior, and keyboard culling now records and displays the last metadata decision after auto-advance so the changed frame remains visible. The imported-grid AX probe exists and now checks selection feedback plus visible inspector rating, but the latest run is blocked by desktop foregrounding: Teststrip launches behind Arc and Accessibility exposes only the application/menu proxy until Teststrip can be made genuinely frontmost.
- Library mockup parity is improving but incomplete. The overview grid now uses cataloged dimensions for true aspect-ratio cells, the Library sidebar exposes a real Recently Added route backed by the latest import output set, the filter rail is closer to the Studio mockup's Ask/search treatment, active Library filter chips, a catalog-backed Sort picker for import order, capture time newest/oldest, and filename, Search active refinement rows, a visible plain-search fallback for unparsed Ask text, and Smart Collection builder rule rows can remove explicit filters, parsed Ask/search chips, saved-set scope, and selected dynamic-set rules without losing structured predicates, the inspector preview size is pinned and fixed above metadata scrolling, the inspector header/metadata controls have initial mockup-derived passes, the culling/loupe chrome now has verdict and filmstrip passes, imported-set stack culling uses real time-adjacent stack counts and explicit visual-similarity vector signals from Apple Vision or local models when present with distance/threshold rationale, persists detected stacks as hidden work-stack sets, can navigate/accept/render stack membership beyond the initial loaded page, writes session Picks output sets after accepted frames exist, and records reviewed/pick/reject decision detail in work history, stack culling can act on a persisted-signal single-frame recommendation or top-two scored keep action and refresh culling work-session progress from decided stack flags, Compare/Survey pick-reject actions now refresh active culling work-session progress and Picks output sets, People has a face-review strip with live queue/scan status, visible Apple Vision scan action, actionable review handoff, manual naming, manual confirmed-person merge, selected-photo face-review dismissal, persisted named rows, and no visible disabled buttons for deferred automatic clustering/split/face-box naming, Search has advanced field parsing, a grouped refine rail, deterministic generated refinements plus provider-signal refinements, deterministic related filters, matching work-history session rows, and existing-workflow suggested actions including current-scope culling, Copilot has real primary actions plus keyword/evaluation/face/OCR/issue/failure review triage over existing queues, Timeline has a catalog-backed year-density ribbon, focused month/day scrubber, and focused scroll syncing, Compare has an eight-frame four-column survey-style pass with metadata badges, active persisted-stack membership, and signal-backed recommendation or neutral ranking copy, Smart Collections has a split builder, removable rule rows, concrete Add Rule presets that compose with selected dynamic saved-set rules, typed freeform rule editing through the existing search parser, contextual deterministic suggestion rows plus provider-signal suggestions for focus/object/OCR/people signals, delete confirmation, and dynamic-to-frozen snapshot path, selected/visible/latest-import/current-scope batch metadata review exists for portable fields with all-catalog confirmation and typed keyword preview chips, selected/current-scope object-label suggestions can be accepted for matching assets across selected photos, the full active query, or a saved set, command/shift page-spanning batches can be frozen into manual saved sets, saved sets can be renamed, duplicated, or deleted with confirmation from the sidebar, dynamic saved sets can be frozen from the sidebar, the sidebar has richer count/detail rows and real review-queue/saved-set/timeline/source-root counts, the top chrome has a first Studio-style pass, and major dead UI gaps plus all designer surfaces are tagged in code, but similarity threshold tuning and broader natural-language Search planning still need visual passes against the design concept.
- The current RAW story has an explicit ImageIO capability matrix and provider boundary plus local real DNG/CR2/RAF fixture coverage, but still lacks CRW/X3F fixture files and a non-ImageIO provider for unsupported or weakly supported families such as Sigma/Foveon X3F. Lytro support remains out of scope.
- Evaluation is scaffolding plus early useful providers, not finished face/person/object/aesthetic workflow. The People view now uses real face-signal coverage for review cards, hands those cards off to existing review targets, can request Apple Vision evaluation for cached previews in the current catalog or search scope, and supports manual naming/merge/dismiss actions for confirmed people state without exposing disabled automated-clustering buttons as normal controls; named or dismissed face-review assets leave the unnamed face-review queues and review summaries without deleting their raw face signals. Local metrics emit focus/exposure/color plus a coarse provisional motion-blur/softness score, local HTTP model output can carry aesthetics/framing labels, object labels can be accepted as selected-photo, selected-batch, visible-batch, latest-import, or full current-scope keyword suggestions, and selected-photo OCR text can be accepted as a caption suggestion, but real automatic face clustering, identity recognition, split, face-box-level naming, richer label review, and broader reprocessing flows are not wired yet.
- Search/sets/work sessions are partially built but not yet the full user-facing model. The Ask/search field now parses a narrow deterministic grammar into catalog predicates, including quoted real-world field values and import/session scope, shows an explicit plain-search fallback when residual natural language remains unparsed, Recent/Starred Work and Recently Added browse `session:<id>` query scopes, related review filters, generated concrete refinements, provider-signal refinements from focus/object/OCR/people evaluation summaries, matching work-history session rows for residual plain text, and a current-scope culling action that starts the existing culling workflow over the visible result scope; culling sessions can produce work-session output sets of accepted frames while snapshotting dynamic inputs for stable history. Clusters, richer query editing, and broader natural-language planning need more implementation.
- Smart collections have a stronger live builder for current filters, removable rule rows, concrete Add Rule preset rows that compose with selected dynamic saved-set rules, typed freeform rule editing through the existing search parser, contextual deterministic suggestion rows, provider-signal suggestions for focus/object/OCR/people evaluation summaries, duplicate/delete saved-set lifecycle, and a dynamic-to-frozen snapshot path. Broader provider-authored query planning is still not complete.
...
- [ ] **Import and source flow:** A user can add an existing folder in place, copy/import from a card-style source, understand preflight failures, see unambiguous progress, open the imported result set, and recover from duplicate/empty/unreadable-source cases without wondering whether the app is stuck. Local fallback import now mirrors worker import by showing the first cataloged frame during progress and keeping the grid anchored while later progress updates continue.
- [x] **Real corpus smoke:** The ignored local corpus under `sample-data/photos/jesse-pictures` imports without committing sample files, catalogs mixed JPEG/DNG/RAF files honestly, preserves originals, and exposes unsupported/partial RAW files without overclaiming preview support. Latest verification: `./script/verify_real_corpus_smoke.sh` scanned 194 candidates, selected/imported/cataloged 3 representative assets, classified 1 working still and 2 best-effort RAWs, reported 0 unsupported files, queued 6 previews, and left selected originals/sidecars unchanged; `./script/build_and_run.sh --verify-real-corpus` also seeded a normal isolated app catalog from the same representative corpus and launched it in the background with 6 cached previews.
...
- [ ] **Culling over arbitrary sets:** Culling works over any saved search, frozen snapshot, manual set, work-session output set, latest import, or detected stack set. Compare/survey supports the current eight-frame layout, can apply Pick/Reject decisions to the active set, and now refreshes active culling work-session progress plus Picks output sets after Compare decisions. Loaded-stack keep/reject decisions now refresh the same active culling progress and Picks output set for normal detected stacks as well as persisted work-stack sets. Stack culling shortcuts are visible in the macOS Culling menu as well as local key capture.
- [ ] **Search, sets, and work sessions:** Users can build and save practical smart collections, freeze snapshots, save manual selections, revisit recent/starred work sessions, search work history by plain text, and treat “photos from this work session” as a queryable set rather than a separate top-level container.
...
- [ ] **Recognition and provisional labels:** Local-first evaluation produces visible provenance-backed signals for faces, objects/content, OCR, focus, motion blur/softness, exposure, color/look, visual similarity, model-provided aesthetics/framing, and provider failures. Machine labels remain provisional until the user accepts them into metadata/XMP; selected object labels can become keywords, batch/current-scope object labels can become keywords, and selected OCR text can become a caption. Apple Vision and local HTTP provider results can now preserve multiple object labels in one signal without auto-writing metadata. Remaining gap: broader live-app verification of recognition/provider-failure review flows and any additional useful signal axes not already surfaced.
- [ ] **Offline and reconnect behavior:** Cached previews allow browsing and culling when originals are on an offline NAS/removable/cloud-backed volume, and reconnect remains reachable from source-root history while preserving unambiguous sidecar paths and source-root identity. Headless cache/reconnect/sidecar preservation is now covered by `./script/verify_offline_reconnect_smoke.sh`, source-root sidebar rows now name unavailable-original counts when a root has offline/missing/moved/stale assets, `Offline Originals` source rows are warning-level rather than destructive, and the selected-photo inspector uses the same cached-preview-only availability copy; remaining work is user-facing browse/cull verification and polish around offline source states.
- [ ] **Worker control:** Long-running import, preview, metadata, source, and recognition work is visible, cancelable, and recoverable. Activity no longer overclaims undispatched worker commands as already running, the user can stop idle worker work, and current-scope recognition requests are capped to a manageable cached-preview batch. Remaining gap: run the worker-control surface through a real app workflow once focus-stealing UI automation is acceptable or Jesse is idle.
- [ ] **Dev build and verification:** The macOS dev app builds reliably with the helper staged, focused tests cover new behavior, smoke verification uses non-focus-stealing captures by default, and focus-stealing UI automation is reserved for explicit approval or idle time. Latest non-focus verification: `swift test` passed with 987 tests, 5 skipped, and 0 failures, and `./script/build_and_run.sh --build` rebuilt and signed `dist/Teststrip.app` plus `TeststripWorker`. Latest full headless workflow gate: `./script/verify_headless_workflows.sh` passed on 2026-07-06 at 06:50 with `955` Swift tests, sandboxed app/helper signing, metadata write, card import, preview drain, source availability, offline reconnect, preview render, local HTTP model, real corpus, and worker recovery gates.
...
- [x] Show queued/building/issue state on imported grid thumbnails that do not have cached previews yet.
...
- [x] Make active Library filter chips removable across explicit filters, parsed Ask/search chips, saved-set scope, and selected dynamic-set rules.
- [x] Make Search active refinement chips and rail rows removable while keeping related filters actionable.
...
- [ ] Add or collect licensed real RAW sample files for Canon CRW and Sigma/Foveon RAW. Local ignored DNG, Canon CR2, and Fuji RAF fixture links under `sample-data/photos/raw-fixtures-local` have been verified from local photo folders through ImageIO metadata/dimension reads.
...
- [x] Make current-scope pending XMP retry find retryable items beyond the loaded thumbnail window.
...
- [x] Surface stack-level Keep recommended / Keep top-ranked guidance in the rapid-cull `TESTSTRIP READS` verdict surface when persisted quality signals rank the active stack.
...
- [x] Remove visible disabled People future-action buttons so deferred automatic clustering, split, and face-box naming stay code-ledger tracked without looking like shipped controls.
...
- [x] Accept selected-photo OCR text as an explicit caption suggestion that writes through catalog-first metadata and XMP sidecars only after user acceptance.
...
- [x] Keep real LM Studio/Ollama endpoint smoke as an optional environment-specific check when one is available.
- [x] Commit.
...
- [x] Make Smart Collection builder rule rows removable without losing structured filter targets.
- [x] Support frozen snapshots separately from dynamic saved searches.
- [x] Surface existing save dynamic set, freeze snapshot, current-scope culling, and non-empty review-queue workflows as Search suggested actions.
...
- [x] Add a repeatable offline reconnect smoke verifier for cached-preview continuity, source-root reconnect, XMP sidecar path migration, and non-destructive source preservation.
...
- [x] Set measured debug-alpha thresholds for larger 500k/1M catalog stress paths after measuring current local behavior.
- [ ] Keep app workflow resource snapshots diagnostic until enough foreground/idle runs exist for honest red/yellow/green thresholds.
