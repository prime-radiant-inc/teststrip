...
- Current slice focused verification: `swift test --filter 'AppModelTests/testRequestCurrentScopeAssetEvaluationsQueuesOnlyBoundedCachedBatch|AppModelTests/testCanRequestCurrentScopeAssetEvaluationsRequiresCatalogAssetsAndWorker|AppModelTests/testCanRequestCurrentScopeAssetEvaluationsRequiresCachedPreviewCandidate|AppModelTests/testRequestCurrentScopeAssetEvaluationsDispatchesCachedAssetsBeyondLoadedPage'` passed after bounding current-scope recognition requests to a cached-preview batch.
- Last focused unit verification: `swift test --filter 'AppModelTests/testRequest.*Evaluation|AppModelTests/testCanRequest.*Evaluation|AppModelTests/testVisibleWorkActivities|WorkerSupervisorTests'` passed with 43 tests after bounding current-scope evaluation queueing and correcting user-facing worker activity status. Earlier focused verification includes latest-import preview status scoping, queued import progress copy, culling command menu presentation, Search refinement removal, removable Library filter chips, preview queue thumbnail state, inspector sizing, offline reconnect smoke, empty import summaries, source reconnect errors, import cancel affordance, Copilot scope actions, culling stack guidance, latest-import evaluation handoff, People review-card lock-state, generated Search refinements, People scan action, saved-set lifecycle, RAW catalog-only import, batch metadata/XMP, culling, and import-path slices listed below.
- Last broad unit verification: `swift test` passed after bounding current-scope evaluation queueing, with 911 tests, 1 skipped, and 0 failures. Earlier `swift test` passed after correcting user-facing worker activity status, with 909 tests, 1 skipped, and 0 failures. Earlier `swift test` passed after scoping latest-import preview status, with 908 tests, 1 skipped, and 0 failures.
- Current slice app workflow verification: No foreground app workflow run was performed for the current-scope evaluation batch cap because Jesse asked to minimize focus-stealing UI automation while actively using the machine; the change is covered by app-model tests, SwiftUI compilation, WorkerSupervisor tests, and full `swift test`.
...
- Current slice: bounded current-scope recognition requests to 40 cached-preview photos per action, disabled the action when the current scope has no cached-preview candidate, and reports when more cached photos remain.
