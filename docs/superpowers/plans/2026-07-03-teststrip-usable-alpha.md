...
- Current slice focused verification: `swift test --filter 'AppModelTests/testBackgroundImportReportsSkippedSourceFilesInCompletionCopy|AppModelTests/testBackgroundImportReportsNoPhotosImportedWhenEverySourceFileIsSkipped|AppModelTests/testWorkerImportCompletionReportsSkippedSourceFiles|ImportCompletionPresentationTests/testSurfacesImportIssuesAsMetricRows|WorkerProtocolTests/testImportCompletionEventRoundTripsImportedAssetIDs|WorkerCommandExecutorTests/testImportFolderCommandReportsSkippedSourceFileCount|CatalogDatabaseTests/testPersistsAndListsWorkSessionsByRecencyAndStarredState'` and the broader import/worker/completion group passed after persisting skipped-file import issue details through local imports, worker imports, catalog work sessions, and the completion presentation. Immediately before that, focused latest-import flagged-review verification passed for opening likely-issue assets from the import output set, focused People verification passed for named people leaving unnamed face-review queues while preserving raw face signals, focused Search/parser verification passed for quoted field parsing and import-batch parsing, and focused recognition verification passed for cached-preview evaluation gates plus retrying failed provider work.
...
- Last broad unit verification: `swift test` passed after the skipped-file import issue slice, with 944 tests, 5 skipped, and 0 failures. Earlier full-suite checks in this run passed with 943, 939, 937, 934, 931, 929, and 926 tests after the latest-import flagged review, People face-review, Search parser, recognition retry/gate, card-import smoke, card-import route, and culling-feedback slices.
- Current slice app workflow verification: `./script/verify_headless_workflows.sh` passed on 2026-07-06 after adding the card-import smoke to the chain. It rebuilt the sandboxed app/helper and ran non-focus metadata write, card import smoke, import preview drain, source availability, offline reconnect, preview render, local HTTP model smoke, and worker recovery checks. No foreground NSOpenPanel smoke was run because Jesse asked to minimize focus-stealing automation while using the machine.
