...
- `abea302`: guarded unreadable XMP conflict sidecars by exposing explicit selected-conflict sidecar state and disabling sidecar-dependent inspector actions while keeping `Use Catalog` available to recreate the sidecar from catalog metadata.
- `3def39f`: protected pending worker-backed XMP writes from stale sidecars by writing catalog metadata over older uncheckpointed sidecars and recording a conflict when the sidecar is newer than the pending row.
- `ce011c0`: made batch-only selected grid cells visibly outlined so command/shift selection no longer depends only on the small top-left badge.
...
- [ ] **Selection, rating, and metadata:** Imported photos can be clicked, selected, rated, labeled, flagged, rejected, keyworded, and inspected with immediate catalog updates and no destructive writes to originals. Recent code-level selection fixes stopped normal grid selection from stealing hidden culling focus and made batch-only selection visibly outlined; remaining gap is live imported-grid verification when foreground UI automation is acceptable.
- [ ] **Catalog-first XMP:** Portable metadata writes catalog first, mirrors automatically to XMP when possible, shows pending/conflict states, supports safe conflict resolution, and leaves originals untouched. Pending worker-backed writes now keep catalog edits authoritative over older uncheckpointed sidecars and record conflicts for newer uncheckpointed sidecars; remaining gap is broader live-app verification of conflict review/resolution workflows.
