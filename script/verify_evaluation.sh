...
TARGET_ASSET="${2:-}"
TIMEOUT_SECONDS="${TESTSTRIP_AX_TIMEOUT_SECONDS:-20}"
...
let timeout = TimeInterval(ProcessInfo.processInfo.environment["TESTSTRIP_AX_TIMEOUT_SECONDS"] ?? "20") ?? 20
...
func button(named label: String) -> AXUIElement? {
    walk(root) { element in
        stringAttribute(element, kAXRoleAttribute) == kAXButtonRole
            && accessibleText(element) == label
    }
}

let thumbnail = walk(root) { element in
...
guard let thumbnail, let selectedName = accessibleText(thumbnail) else {
    if requestedTarget.isEmpty {
...
let selectResult = AXUIElementPerformAction(thumbnail, kAXPressAction as CFString)
guard selectResult == .success else {
    fputs("AXPress failed for \(selectedName): \(selectResult.rawValue)\n", stderr)
    exit(1)
}

guard let evaluateButton = button(named: "Evaluate") else {
    fputs("Evaluate button not found in \(appName)\n", stderr)
    exit(1)
}

let evaluateResult = AXUIElementPerformAction(evaluateButton, kAXPressAction as CFString)
guard evaluateResult == .success else {
    fputs("AXPress failed for Evaluate: \(evaluateResult.rawValue)\n", stderr)
    exit(1)
}
...
guard waitFor({
    walk(root) { element in
        stringAttribute(element, kAXRoleAttribute) == kAXStaticTextRole
            && accessibleText(element) == "Signals"
    } != nil
}) else {
    fputs("Evaluation signals did not become visible within \(timeout)s\n", stderr)
    exit(1)
}

print("evaluation signals visible for \(selectedName)")
'
