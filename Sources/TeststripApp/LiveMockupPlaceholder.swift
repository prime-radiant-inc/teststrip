...
public enum LiveMockupPlaceholders {
    ...
    public static let keywordingBatch = LiveMockupPlaceholder(
        ...
        currentFallback: "Single-selected metadata controls with removable selected keyword chips plus latest-import keyword review, current-scope keyword suggestions, freeform keyword entry, and a selected/visible/current-scope metadata popover write command and shift selected assets through catalog and XMP with all-catalog confirmation."
    )
    ...
    public static let compareSurvey = LiveMockupPlaceholder(
        ...
        currentFallback: "Four-column 4x2 survey grid over up to eight frames from persisted culling stack sets when active, loaded-scope candidate stacks otherwise, or the current selected neighborhood with heuristic recommendation text and manual culling handoff."
    )
