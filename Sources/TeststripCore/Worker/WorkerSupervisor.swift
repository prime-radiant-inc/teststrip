...
public final class WorkerSupervisor: @unchecked Sendable {
    ...
    public func enqueue(
        ...
        try enqueue([(item: item, command: command, placement: placement)])
    }

    public func enqueue(
        _ requests: [(item: BackgroundWorkItem, command: WorkerCommand, placement: BackgroundWorkQueuePlacement)]
    ) throws {
    ...
    private func handleOutputLine(_ line: String) {
        ...
        case .completedImport(let itemID, let message, _, _, _, _, _):
            guard let itemID else { return }
            completeDispatchedItem(id: itemID, detail: message, event: event)
